...
public class GridViewWithHeaderAndFooter extends GridView {
    ...
    private ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo> mHeaderViewInfos = new ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo>();
    private ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo> mFooterViewInfos = new ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo>();
    private ListAdapter mOriginalAdapter;
    private GridViewWithHeaderAndFooter.ItemClickHandler mItemClickHandler;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (adapter != null && adapter instanceof GridViewWithHeaderAndFooter.HeaderViewGridAdapter) {
            ((GridViewWithHeaderAndFooter.HeaderViewGridAdapter) adapter).setNumColumns(getNumColumnsCompatible());
            ((GridViewWithHeaderAndFooter.HeaderViewGridAdapter) adapter).setRowHeight(getRowHeight());
        }
    }
    ...
    public void addHeaderView(View v, Object data, boolean isSelectable) {
        ...
        FrameLayout fl = new GridViewWithHeaderAndFooter.FullWidthFixedViewLayout(getContext());
    ...
    private void removeFixedViewInfo(View v, ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo> where) {
        int len = where.size();
        for (int i = 0; i < len; ++i) {
            GridViewWithHeaderAndFooter.FixedViewInfo info = where.get(i);
            if (info.view == v) {
                where.remove(i);
                break;
            }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
            GridViewWithHeaderAndFooter.HeaderViewGridAdapter headerViewGridAdapter = new GridViewWithHeaderAndFooter.HeaderViewGridAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
            int numColumns = getNumColumnsCompatible();
            if (numColumns > 1) {
    ...
    private static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
        ...
        static final ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo> EMPTY_INFO_LIST =
                new ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo>();
        ...
        private boolean areAllListInfosSelectable(ArrayList<GridViewWithHeaderAndFooter.FixedViewInfo> infos) {
            if (infos != null) {
                for (GridViewWithHeaderAndFooter.FixedViewInfo info : infos) {
        ...
        public boolean removeHeader(View v) {
            for (int i = 0; i < mHeaderViewInfos.size(); i++) {
                GridViewWithHeaderAndFooter.FixedViewInfo info = mHeaderViewInfos.get(i);
                if (info.view == v) {
        ...
        public boolean removeFooter(View v) {
            for (int i = 0; i < mFooterViewInfos.size(); i++) {
                GridViewWithHeaderAndFooter.FixedViewInfo info = mFooterViewInfos.get(i);
                if (info.view == v) {
    ...
    private GridViewWithHeaderAndFooter.ItemClickHandler getItemClickHandler() {
        if (mItemClickHandler == null) {
            mItemClickHandler = new GridViewWithHeaderAndFooter.ItemClickHandler();
        }
